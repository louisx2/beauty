...
import { Phone, Search, Calendar, Clock, User, Sparkles, XCircle, CheckCircle2, AlertCircle, ArrowLeft, MessageCircle } from 'lucide-react';
...
function canCancel(dateStr: string, timeStr: string): boolean {
  ...
  return diff > 12 * 60 * 60 * 1000; // 12 horas
}
...
export default function MyAppointments() {
  ...
  const upcoming = appointments.filter(a => !isPast(a.date, a.time) && a.status !== 'cancelled' && a.status !== 'completed' && a.status !== 'no_show');
  const past = appointments.filter(a => isPast(a.date, a.time) || a.status === 'cancelled' || a.status === 'completed' || a.status === 'no_show');
    ...
    <section className="my-appts" id="mis-citas">
      <div className="my-appts__inner">
        ...
        <form className="my-appts__search glass" onSubmit={handleSearch} id="track-form">
          ...
          <button type="submit" className="btn-primary" disabled={loading} id="track-submit">
            <Search size={16} />
          ...
          <div className="my-appts__results animate-fade-up">
            {appointments.length === 0 ? (
              <div className="my-appts__empty">
                ...
                <p>No hay citas registradas con el número <strong>{phone}</strong>. Si crees que es un error, contáctanos por WhatsApp.</p>
                <a
                  href={`https://wa.me/18293224014?text=${encodeURIComponent(`Hola, no encuentro mis citas con el número ${phone}`)}`}
                  target="_blank"
              ...
              <>
                <div className="my-appts__welcome">
                  ...
                  <p>{upcoming.length} cita{upcoming.length !== 1 ? 's' : ''} próxima{upcoming.length !== 1 ? 's' : ''} · {past.length} en historial</p>
                </div>
                  ...
                  <div className="my-appts__section">
                    ...
                    <div className="my-appts__list">
                      {upcoming.map(a => {
                        ...
                        return (
                          <div className={`my-appt-card my-appt-card--upcoming my-appt-card--${a.status}`} key={a.id}>
                            <div className="my-appt-card__date-col">
                              <span className="my-appt-card__day">{new Date(`${a.date}T12:00:00`).getDate()}</span>
                              <span className="my-appt-card__month">{new Date(`${a.date}T12:00:00`).toLocaleDateString('es-DO', { month: 'short' })}</span>
                            </div>
                            <div className="my-appt-card__info">
                              <strong>{a.service}</strong>
                              <span><User size={13} /> {a.employee}</span>
                              <span><Clock size={13} /> {a.time} · {a.duration} min</span>
                              <span className="my-appt-card__full-date">{formatDate(a.date)}</span>
                            </div>
                            <div className="my-appt-card__right">
                              <span className="my-appt-card__status" style={{ background: `${st.color}18`, color: st.color }}>
                                {st.icon} {st.label}
                              </span>
                  ...
                  <div className="my-appts__section">
                    ...
                    <div className="my-appts__list">
                      {past.map(a => {
                        ...
                        return (
                          <div className={`my-appt-card my-appt-card--past my-appt-card--${a.status}`} key={a.id}>
